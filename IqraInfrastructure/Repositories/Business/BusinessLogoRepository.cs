...
namespace IqraInfrastructure.Repositories.Business
{
    public class BusinessLogoRepository
    {
        ...
        public BusinessLogoRepository(ILogger<BusinessLogoRepository> logger, S3StorageClientFactory clientFactory)
        {
            _logger = logger;
        ...
        public async Task PutFileAsStreamData(string fileId, Stream fileStream, Dictionary<string, string> metaData, string? region = null)
        {
            ...
            {
                _logger.LogError(ex, "Error putting logo file stream {FileId}", fileId);
                throw;
            }
        }
        ...
        public async Task<MemoryStream> GetFileAtPath(string fileId, string filePath, string? region = null)
        {
            try
            {
                ...
                // Rewind again to return the stream to caller
                memoryStream.Position = 0;
                return memoryStream;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting logo file {FileId} at path {FilePath}", fileId, filePath);
                throw;
            }
        }
        ...
        public async Task<bool> FileExists(string fileId, string? region = null)
        {
            ...
            {
                _logger.LogError(ex, "Error checking existence for logo file {FileId}", fileId);
                return false;
            }
        }
        ...
        public async Task<MemoryStream> GetFileAsMemoryStream(string fileId, string? region = null)
        {
            ...
            {
                _logger.LogError(ex, "Error getting logo file stream {FileId}", fileId);
                throw;
            }
        }
