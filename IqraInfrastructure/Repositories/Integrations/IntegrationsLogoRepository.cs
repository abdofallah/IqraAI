...
namespace IqraInfrastructure.Repositories.Integrations
{
    public class IntegrationsLogoRepository
    {
        ...
        public IntegrationsLogoRepository(ILogger<IntegrationsLogoRepository> logger, S3StorageClientFactory clientFactory)
        {
            _logger = logger;
            _s3StorageClientFactory = clientFactory;
            _bucketName = S3StorageBucketConstants.IntegrationsLogoRepositoryBucketName;
        }
        ...
        public async Task PutFileAsStreamData(string fileId, Stream fileStream, Dictionary<string, string> metaData)
        {
            ...
            {
                _logger.LogError(ex, "Error putting integration logo stream {FileId}", fileId);
                throw;
            }
        }
        ...
        public async Task<MemoryStream> GetFileAtPath(string fileId, string filePath)
        {
            try
            {
                ...
                // Reset again for return
                memoryStream.Position = 0;
                return memoryStream;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting integration logo {FileId} at path {FilePath}", fileId, filePath);
                throw;
            }
        }
        ...
        public async Task<bool> FileExists(string fileId)
        {
            try
            {
                ...
                // Check existence using Metadata (HeadObject)
            ...
            {
                _logger.LogError(ex, "Error checking existence for integration logo {FileId}", fileId);
                return false;
            }
        }
        ...
        public async Task<MemoryStream> GetFileAsMemoryStream(string fileId)
        {
            ...
            {
                _logger.LogError(ex, "Error getting integration logo stream {FileId}", fileId);
                throw;
            }
        }
