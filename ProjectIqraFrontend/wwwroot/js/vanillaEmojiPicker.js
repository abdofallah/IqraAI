const EmojiPicker = function (options) {
	...
	this.closeOnInsert = this.options.closeOnInsert;
	...
	const functions = {
		...
		insert: (e) => {
			...
			if (this.options.closeOnInsert) {
				functions.closePicker.call(this, e);
			}

			this.trigger.forEach((trigger) => {
				if (trigger.startsWith("#")) {
					document.getElementById(trigger.substring(1)).dispatchEvent(
						new CustomEvent("emojiSelected", {
							detail: {
								emoji: emoji,
							},
						}),
					...
					document.getElementsByClassName(trigger.substring(1))[0].dispatchEvent(
						new CustomEvent("emojiSelected", {
							detail: {
								emoji: emoji,
							},
