...
exports.ScrollerImpl = void 0;
const x6_1 = require("@antv/x6");
class ScrollerImpl extends x6_1.View {
    get graph() {
    ...
    constructor(options) {
        ...
        this.onUpdate = x6_1.FunctionExt.debounce(this.onUpdate, 200);
        ...
        x6_1.Dom.addClass(this.container, this.prefixClassName(ScrollerImpl.containerClass));
        x6_1.Dom.css(this.container, { width, height });
        if (this.options.pageVisible) {
            x6_1.Dom.addClass(this.container, this.prefixClassName(ScrollerImpl.pagedClass));
        }
        ...
        if (graphContainer.parentNode) {
            x6_1.Dom.before(graphContainer, this.container);
        }
        this.content = document.createElement('div');
        x6_1.Dom.addClass(this.content, this.prefixClassName(ScrollerImpl.contentClass));
        x6_1.Dom.css(this.content, {
            width: this.graph.options.width,
            height: this.graph.options.height,
        });
        // custom background
        this.background = document.createElement('div');
        x6_1.Dom.addClass(this.background, this.prefixClassName(ScrollerImpl.backgroundClass));
        x6_1.Dom.append(this.content, this.background);
        if (!this.options.pageVisible) {
            x6_1.Dom.append(this.content, this.graph.view.grid);
        }
        x6_1.Dom.append(this.content, graphContainer);
    ...
    delegateBackgroundEvents(events) {
        const evts = events || x6_1.GraphView.events;
    ...
    beforeManipulation() {
        if (x6_1.Platform.IS_IE || x6_1.Platform.IS_EDGE) {
            x6_1.Dom.css(this.container, { visibility: 'hidden' });
        }
    }
    afterManipulation() {
        if (x6_1.Platform.IS_IE || x6_1.Platform.IS_EDGE) {
            x6_1.Dom.css(this.container, { visibility: 'visible' });
        }
    }
        ...
        if (this.options.pageVisible && this.options.pageBreak) {
            ...
            if (graphWidth > pageWidth || graphHeight > pageHeight) {
                ...
                for (let i = 1, l = Math.floor(graphWidth / pageWidth); i < l; i += 1) {
                    ...
                    x6_1.Dom.addClass(div, this.prefixClassName(`graph-pagebreak-vertical`));
                    x6_1.Dom.css(div, { left: i * pageWidth });
                    x6_1.Dom.appendTo(div, container);
                    hasPageBreak = true;
                }
                for (let i = 1, l = Math.floor(graphHeight / pageHeight); i < l; i += 1) {
                    const div = document.createElement('div');
                    x6_1.Dom.addClass(div, this.prefixClassName(`graph-pagebreak-horizontal`));
                    x6_1.Dom.css(div, { top: i * pageHeight });
                    x6_1.Dom.appendTo(div, container);
                    hasPageBreak = true;
                }
                if (hasPageBreak) {
                    x6_1.Dom.addClass(container, this.prefixClassName('graph-pagebreak'));
                    x6_1.Dom.after(this.graph.view.grid, container);
    ...
    update() {
        ...
        if (typeof resizeOptions === 'function') {
            resizeOptions = x6_1.FunctionExt.call(resizeOptions, this, this);
        }
    ...
    getFitToContentOptions(options) {
        ...
        if (!this.options.autoResize) {
            options.contentArea = x6_1.Rectangle.create();
        }
        return options;
    }
    updateScale(sx, sy) {
    ...
    scrollbarPosition(left, top) {
        ...
        x6_1.Dom.prop(this.container, prop);
        return this;
    }
    /**
    ...
    positionRect(rect, pos, options) {
        const bbox = x6_1.Rectangle.create(rect);
        switch (pos) {
            case 'center':
    ...
    positionPoint(point, x, y, options = {}) {
        ...
        const padding = x6_1.NumberExt.normalizeSides(pad);
        const clientRect = x6_1.Rectangle.fromSize(this.getClientSize());
        ...
        x = x6_1.NumberExt.normalizePercentage(x, Math.max(0, targetRect.width));
        if (x < 0) {
            x = targetRect.width + x; // eslint-disable-line
        }
        // eslint-disable-next-line
        y = x6_1.NumberExt.normalizePercentage(y, Math.max(0, targetRect.height));
        if (y < 0) {
        ...
        const result = x6_1.Point.create(point).translate(rawDiff);
    ...
    zoomToRect(rect, options = {}) {
        const area = x6_1.Rectangle.create(rect);
        const graph = this.graph;
    ...
    transitionToPoint(x, y, options) {
        ...
        const clientSize = this.getClientSize();
        const targetPoint = new x6_1.Point(x, y);
        ...
        x6_1.Dom.addClass(this.container, ScrollerImpl.transitionClassName);
        x6_1.Dom.Event.off(this.content, ScrollerImpl.transitionEventName);
        x6_1.Dom.Event.on(this.content, ScrollerImpl.transitionEventName, (e) => {
            this.syncTransition(targetScale, { x: x, y: y });
            if (typeof onTransitionEnd === 'function') {
                x6_1.FunctionExt.call(onTransitionEnd, this, e.originalEvent);
            }
        });
        x6_1.Dom.css(this.content, {
            transform,
    ...
    removeTransition() {
        x6_1.Dom.removeClass(this.container, ScrollerImpl.transitionClassName);
        x6_1.Dom.Event.off(this.content, ScrollerImpl.transitionEventName);
        x6_1.Dom.css(this.content, {
            transform: '',
            transformOrigin: '',
    ...
    transitionToRect(rectangle, options = {}) {
        const rect = x6_1.Rectangle.create(rectangle);
        ...
        const center = options.center
            ? x6_1.Point.create(options.center)
            : rect.getCenter();
        const clientSize = this.getClientSize();
        const w = clientSize.width * PIXEL_SIZE;
        const h = clientSize.height * PIXEL_SIZE;
        let scale = new x6_1.Rectangle(center.x - w / 2, center.y - h / 2, w, h).getMaxUniformScaleToFit(rect, center);
    ...
    startPanning(evt) {
        ...
        x6_1.Dom.Event.on(document.body, {
            'mousemove.panning touchmove.panning': this.pan.bind(this),
            'mouseup.panning touchend.panning': this.stopPanning.bind(this),
            'mouseleave.panning': this.stopPanning.bind(this),
        });
        x6_1.Dom.Event.on(window, 'mouseup.panning', this.stopPanning.bind(this));
    }
    pan(evt) {
        const e = this.normalizeEvent(evt);
    ...
    stopPanning(e) {
        x6_1.Dom.Event.off(document.body, '.panning');
        x6_1.Dom.Event.off(window, '.panning');
    ...
    clientToLocalPoint(a, b) {
        ...
        return new x6_1.Point(x / ctm.a, y / ctm.d);
    }
    localToBackgroundPoint(x, y) {
        const p = typeof x === 'object' ? x6_1.Point.create(x) : new x6_1.Point(x, y);
        const ctm = this.graph.matrix();
        const padding = this.padding;
        return x6_1.Util.transformPoint(p, ctm).translate(padding.left, padding.top);
    }
    resize(width, height) {
        ...
        x6_1.Dom.css(this.container, { width: w, height: h });
        this.update();
    }
    getClientSize() {
    ...
    getPadding() {
        ...
        if (typeof padding === 'function') {
            return x6_1.NumberExt.normalizeSides(x6_1.FunctionExt.call(padding, this, this));
        }
        return x6_1.NumberExt.normalizeSides(padding);
    }
    /**
    ...
    getVisibleArea() {
        ...
        const area = x6_1.Util.transformRectangle(box, ctm.inverse());
     ...
     */
    lock() {
        x6_1.Dom.css(this.container, { overflow: 'hidden' });
        return this;
    }
    /**
     ...
     */
    unlock() {
        x6_1.Dom.css(this.container, { overflow: 'scroll' });
        return this;
    }
    onRemove() {
        this.stopListening();
    ...
    dispose() {
        x6_1.Dom.before(this.container, this.graph.container);
        this.remove();
    }
}
__decorate([
...
(function (ScrollerImpl) {
    class Background extends x6_1.BackgroundManager {
        get elem() {
...
})(ScrollerImpl = exports.ScrollerImpl || (exports.ScrollerImpl = {}));
(function (ScrollerImpl) {
    ...
    function getOptions(options) {
        const result = x6_1.ObjectExt.merge({}, ScrollerImpl.defaultOptions, options);
