...
require("./api");
class Scroller extends x6_1.Basecoat {
    get pannable() {
        if (this.options) {
    ...
    init(graph) {
        this.graph = graph;
        const options = scroller_1.ScrollerImpl.getOptions(Object.assign(Object.assign({ enabled: true }, this.options), { graph }));
        this.options = options;
    ...
    startListening() {
        ...
        if (eventTypes.includes('rightMouseDown')) {
            ...
            x6_1.Dom.Event.on(this.scrollerImpl.container, 'mousedown', this.onRightMouseDown);
        }
    }
    stopListening() {
        let eventTypes = [];
        ...
        if (eventTypes.includes('rightMouseDown')) {
            x6_1.Dom.Event.off(this.scrollerImpl.container, 'mousedown', this.onRightMouseDown);
        }
    }
    onRightMouseDown(e) {
    ...
    allowPanning(e, strict) {
        return (this.pannable && x6_1.ModifierKey.isMatch(e, this.options.modifiers, strict));
    }
    updateClassName(isPanning) {
        const container = this.scrollerImpl.container;
        const pannable = x6_1.Config.prefix('graph-scroller-pannable');
        if (this.pannable) {
            x6_1.Dom.addClass(container, pannable);
            ...
            x6_1.Dom.removeClass(container, pannable);
        }
    }
    dispose() {
