import { ArrayExt, Dom, FunctionExt } from "@antv/x6-common";
import { GeometryUtil, Point, Rectangle } from "@antv/x6-geometry";
...
> extends CellView<Entity, Options> {
	...
	onMouseDown(e: Dom.MouseDownEvent, x: number, y: number) {
		const isImmovable = this.isElementOrParentImmovable(e.target);
	...
	private isElementOrParentImmovable(element: HTMLElement): boolean {
		let currentElement: HTMLElement | null = element;

		while (currentElement) {
			// Check if the current element is immovable
			if (currentElement.classList.contains("html-shape-immovable")) {
				return true;
			}

			// Stop traversing if we hit the x6-cell or x6-node parent
			if (
				currentElement.classList.contains("x6-cell") ||
				currentElement.classList.contains("x6-node")
			) {
				break;
			}
