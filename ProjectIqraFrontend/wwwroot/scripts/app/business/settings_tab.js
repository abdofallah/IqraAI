...
let IsSavingUsersTab = false;
let IsManageUserTabOpened = false;
let ManageUserType = null;
let CurrentManageSubUserData = null;
...
function FillSettingsUsersManageTabPermissionsTab(usersDataPermissions) {
	function FillRoutings() {
		const routingPermissions = usersDataPermissions.routing;
	...
	function FillTools() {
		const toolsPermissions = usersDataPermissions.tools;
	...
	function FillUsers() {
		const agentsPermissions = usersDataPermissions.agents;
	...
	function FillContext() {
		const contextPermissions = usersDataPermissions.context;
	...
	function FillMakeCalls() {
		const makeCallsPermissions = usersDataPermissions.makeCalls;
	...
	function FillConversations() {
		const conversationsPermissions = usersDataPermissions.conversations;
	...
	function FillSettings() {
		const settingsPermissions = usersDataPermissions.settings;
...
function ValidateSettingsSubusersGeneralFields(onlyRemove = true) {
	...
	const password = businessSubuserPassword.val().trim();
	if (password === "") {
		validated = false;
...
function CheckSettingsSubusersGeneralTabHasChanges() {
	...
	if (isLoginDisableChecked === true && businessSubuserLoginDisabledReasonInput.val() !== CurrentManageSubUserData.disabledUserLoginReason) {
		hasChanges = true;
	}
...
function CheckIfSettingsSubusersManageHasChanges(enableDisableButton = true) {
	const subusersGeneralTabChanges = CheckSettingsSubusersGeneralTabHasChanges();
	const subusersPermissionTabChanges = CheckSettingsSubusersPermissionsTabHasChanges();
	const subusersWhiteLabelTabChanges = CheckSettingsSubusersWhiteLabelHasChanges();

	const hasChanges = subusersGeneralTabChanges.hasChanges || subusersPermissionTabChanges.hasChanges || subusersWhiteLabelTabChanges.hasChanges;
...
function CheckIfSettingsDomainManagerHasChanges(enableDisableButton = true) {
	...
	const currentDomainType = businessDomainsDomainType.val();
	...
	if (currentDomainType === "IqraSubdomain") {
		...
		const checkedSSLType = businessDomainsSslConfig.find('input[name="businessDomainSSLType"]:checked').attr("ssl-type");
...
function ValidateSettingsDomainManageFields(onlyRemove = true) {
	...
	const domainType = businessDomainsDomainType.val();
	...
	if (domainType === "CustomDomain") {
		...
		const domainSSLType = businessDomainsSslConfig.find('input[name="businessDomainSSLType"]:checked').attr("ssl-type");
...
function initSettingsTab() {
	$(document).ready(() => {
		...
		addNewBusinessSubuserButton.on("click", (event) => {
			...
			const newObject = CreateSettingsDefaultSubuserObject();
		...
		businessSubuserWhiteLabelLogo.on("change", (event) => {
			...
			const file = businessSubuserWhiteLabelLogo[0].files[0];
			if (!file) {
				return;
			}
		...
		businessSubuserWhiteLabelFavicon.on("change", (event) => {
			...
			const file = businessSubuserWhiteLabelFavicon[0].files[0];
			if (!file) {
				return;
			}
		...
		$("#nav-bar").on("tabChange", async (event) => {
			...
			if (IsManagerDomainTabOpened) {
				...
				const manageDomainChanges = CheckIfSettingsDomainManagerHasChanges(false).hasChanges;
				if (!manageDomainChanges) {
					switchBackToBusinessDomainsTab.click();
				} else {
					const confirmCloseMangeDomainDialog = new BootstrapConfirmDialog({
			...
			if (IsManageUserTabOpened) {
				...
				const manageSubuserChanges = CheckIfSettingsSubusersManageHasChanges(false);
				if (!manageSubuserChanges) {
					switchBackToBusinessSubusersTab.click();
				} else {
					const confirmCloseMangeUserDialog = new BootstrapConfirmDialog({
					...
					const confirmCloseMangeUserResult = await confirmCloseMangeUserDialog.show();
			...
			const settingsChanges = CheckIfSettingsHasChanges(false);
		...
		saveBusinessSubuserButton.on("click", (event) => {
			...
			const whiteLabelGeneralTabValidation = ValidateSettingsSubusersWhiteLabelGeneralFields(false);
			...
			const subUsersTabChanges = CheckIfSettingsSubusersManageHasChanges(false);
		...
		$("#subusersManagerTab input[check-type=permission-with-reason]").on("change", (event) => {
			...
			const reasonInput = current.parent().parent().find("input[type=text]");
		...
		addNewBusinessDomainButton.on("click", (event) => {
			...
			const newObject = CreateSettingsDefaultDomainObject();
		...
		businessDomainsSslConfig.on("change", 'input[name="businessDomainSSLType"]', (event) => {
			...
			const current = $(event.currentTarget);
			const currentCheckedSSLType = current.attr("ssl-type");
		...
		saveBusinessDomainButton.on("click", (event) => {
			...
			const manageTabValidation = ValidateSettingsDomainManageFields(false);
			...
			const domainManagerChanges = CheckIfSettingsDomainManagerHasChanges(false);

			formData.append("changes", JSON.stringify(domainManagerChanges.changes));

			SaveSettingsBusinessDomain(
				formData,
				(saveResponse) => {
					console.log(saveResponse);

					const newDomainData = saveResponse.data;

					if (ManageDomainType === "edit") {
						const updatedDomainCurrentIndex = BusinessFullData.businessWhiteLabelDomain.findIndex((x) => x.id === CurrentManageDomainData.id);
		...
		businessSubusersTable.on("click", 'button[button-type="settingsUserEdit"]', (event) => {
			...
			const currentSubuserEmail = current.attr("user-email");

			const currentSubuserData = BusinessFullData.businessData.subUsers.find((x) => x.email === currentSubuserEmail);
